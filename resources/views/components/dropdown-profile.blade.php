...
<div class="relative inline-flex" x-data="{ open: false }">
    <button
        ...
        :aria-expanded="open"                        
    >
        <img class="w-8 h-8 rounded-full" src="https://ui-avatars.com/api/?name=a&color=7F9CF5&background=EBF4FF" width="32" height="32" alt="{{ Auth::user()->name }}" />
        ...
        <ul>
            <li>
                <form method="POST" action="{{ route('logout') }}" x-data>
                    ...
                    <button class="flex items-center py-1 px-3 font-medium text-sm text-primary-500 hover:text-primary-300 dark:hover:text-primary-500/80 "
                    type="submit"
                    >
                        {{ __('Log Out') }}
                    </button>
